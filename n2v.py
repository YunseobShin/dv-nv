...
def train(input_file, output_file):
    ...
    node2vec = Node2Vec(graph, dimensions=128, walk_length=80, num_walks=20, workers=8, p=0.5, q=0.5)
    # np.save(output_file + 'walks', node2vec.walks)
    model = node2vec.fit(window=4, min_count=1, batch_words=4)  # Any keywords acceptable by gesim.Word2Vec can be passed, `diemnsions` and `workers` are automatically passed (from the Node2Vec constructor)
